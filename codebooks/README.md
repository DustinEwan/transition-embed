...
trained on **wikitext-103** with a **KDA** backbone (the conv-like, short-range
...
| **code_dim** | 512 |
| **backbone** | KDA (single layer, hidden = code_dim) |
| **corpus** | wikitext-103-v1, 124.3M tokens |
| **recipe** | weighted CE (inverse-freq) + stratified VISReg + online unigram centering |
| **interface** | tied binary: input = raw values, output = sign (2-bit), unigram-centered by construction |
...
~6–7 token *reach* is a property of the KDA backbone, not of the library.
